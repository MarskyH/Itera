<template>
  <Form @submit="handleSubmit" @invalid-submit="onInvalid" :validation-schema="schema">
    <main class="py-8 flex items-center justify-center min-h-screen"
      ...
      <div class="flex flex-col items-center w-[800px] h-[750px] shadow-md p-6 rounded-md bg-white dark:bg-onyx-900">
        ...
        <div class="flex flex-col w-full items-center mb-4">
          ...
          <CustomInput name="nome" label="Nome Completo" class-helper-text="text-lightSilver-900" :required="true"
            :max-length="100" :icon-path="mdiAccount" />
          <CustomInput name="login" label="Usuário" class-helper-text="text-lightSilver-900" :required="true"
            :max-length="16" :icon-path="mdiAccount" />
          <CustomInput name="email" type="email" label="E-mail" helper-text="" class-helper-text="text-lightSilver-900"
            :icon-path="mdiEmail" :required="true" :max-length="50" />
          <CustomInput name="password" label="Senha" helper-text="" class-helper-text="text-lightSilver-900"
            :type="showPassword ? 'text' : 'password'" :required="true" :icon-path="mdiLock" :max-length="80" />

          <CustomInput name="confirmationPassword" label="Confirme Senha" :type="showPassword ? 'text' : 'password'"
            :required="true" :icon-path="mdiLock" :max-length="80" />
